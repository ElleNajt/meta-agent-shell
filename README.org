...
note-for-later desc "note text"         # leave timestamped note in .tasks/
#+end_src
...
Alternative: Run everything in a VM with limited credentials. This requires =bypassPermissions= mode since the permission prompts don't work well with multi-agent coordination.
