...
This teaches all agents how to communicate with each other and search across sessions. Add to =~/.claude/CLAUDE.md=:
...
| ~(meta-agent-shell-search-sessions "pattern")~                | Search all sessions for pattern |
...
The heartbeat sends periodic status updates to the meta-agent with all active sessions and your standing instructions. Useful if you want the meta-agent to proactively monitor and coordinate.

** Setup

1. Create a heartbeat file with your standing instructions:

#+begin_src bash
echo "Check on agent progress. Alert me if any agent seems stuck." > ~/heartbeat.org
#+end_src
...
(setq meta-agent-shell-heartbeat-interval 900)  ; seconds (default: 15 min)
(setq meta-agent-shell-heartbeat-cooldown 300)  ; delay after user interaction
#+end_src

3. Start the heartbeat timer:

#+begin_example
M-x meta-agent-shell-heartbeat-start
#+end_example

The meta-agent receives a formatted status of all sessions plus your instructions at each interval.
